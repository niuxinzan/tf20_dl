...
def DarkNet52(num_class):
    inputs = layers.Input(shape=(416, 416, 3))
    num_class = num_class
    dbl_block_0 = DBL_block(filters=32, kernel_size=(3, 3), strides=1)
    down_sample_0 = DBL_block(filters=64, kernel_size=(3, 3), strides=2)

    Residual_block_1 = Residual_block(32, 1)
    down_sample_1 = DBL_block(filters=128, kernel_size=(3, 3), strides=2)

    Residual_block_2 = Residual_block(64, 1)
    down_sample_2 = DBL_block(filters=256, kernel_size=(3, 3), strides=2)

    Residual_block_3 = Residual_block(128, 1)
    down_sample_3 = DBL_block(filters=512, kernel_size=(3, 3), strides=2)

    Residual_block_4 = Residual_block(256, 1)
    down_sample_4 = DBL_block(filters=1024, kernel_size=(3, 3), strides=2)

    Residual_block_5 = Residual_block(512, 1)

    Conv_set_block_1 = Conv_set_block(512)
    Conv_1_1 = DBL_block(filters=1024, kernel_size=(3, 3), strides=1)
    Conv_1_2 = layers.Conv2D(filters=3 * (num_class + 5), kernel_size=1, strides=1)

    Conv_1_1_1 = DBL_block(filters=256, kernel_size=(1, 1), strides=1)
    upsample_1_1 = layers.UpSampling2D()

    Conv_set_block_2 = Conv_set_block(256)
    Conv_2_1 = DBL_block(filters=512, kernel_size=(3, 3), strides=1)
    Conv_2_2 = layers.Conv2D(filters=3 * (num_class + 5), kernel_size=1, strides=1)

    Conv_2_1_1 = DBL_block(filters=128, kernel_size=(1, 1), strides=1)
    upsample_2_1 = layers.UpSampling2D()

    Conv_set_block_3 = Conv_set_block(128)
    Conv_3_1 = DBL_block(filters=256, kernel_size=(3, 3), strides=1)
    Conv_3_2 = layers.Conv2D(filters=3 * (num_class + 5), kernel_size=1, strides=1)
    x = dbl_block_0(inputs)
    x = down_sample_0(x)
    ...
    x = Residual_block_3(x)

    route_1 = x
    x = down_sample_3(x)
    x = Residual_block_4(x)

    route_2 = x
    x = down_sample_4(x)

    x = Residual_block_5(x)
    conv = x

    conv = Conv_set_block_1(conv)
    conv_lobj_branch = Conv_1_1(conv)
    conv_lbbox = Conv_1_2(conv_lobj_branch)

    conv = Conv_1_1_1(conv)
    conv = upsample_1_1(conv)
    conv = tf.concat([conv, route_2], axis=-1)

    conv = Conv_set_block_2(conv)
    conv_mobj_branch = Conv_2_1(conv)
    conv_mbbox = Conv_2_2(conv_mobj_branch)

    conv = Conv_2_1_1(conv)
    conv = upsample_2_1(conv)
    conv = tf.concat([conv, route_1], axis=-1)
    conv = Conv_set_block_3(conv)
    conv_sobj_branch = Conv_3_1(conv)
    conv_sbbox = Conv_3_2(conv_sobj_branch)
    return Model(inputs,[conv_lbbox,conv_mbbox,conv_sbbox])
'''
ValueError: You cannot build your model by calling `build` if your layers do not support float type inputs. Instead, in order to instantiate and build your model, `call` your model on real tensor data (of the correct dtype).
'''
class DarkNet52_1(tf.keras.Model):
...
# def yolov3():
#     return DarkNet52(num_class=10)
# ss=yolov3()
# ss.build(input_shape=(416,416,3))
# ss.summary()
