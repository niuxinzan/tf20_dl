...
from networks_mobile.mobilenet import mobilenet_encoder
...
# 语义分割网络SegNet
def SegNet_Mobile(input_height=416, input_width=416, n_classes=2, n_upSample=3, encoder_level=3):

    img_input, features = mobilenet_encoder(input_height=input_height, input_width=input_width)
    feature = features[encoder_level]  # (26,26,512)
    output = decoder(feature, n_classes, n_upSample)

    # 将结果进行reshape
    output = tf.reshape(output, (-1, int(input_height / 2) * int(input_width / 2), n_classes))
